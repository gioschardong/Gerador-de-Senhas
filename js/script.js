...
// Remover cores aplicadas ao medidor de força
...
// Preenche as barras com a cor expecifica
...
// Display de texto para descrição da força da senha e
// preenche as barras em questão
const styleMeter = (rating) => {
  ...
  switch(numBars) {
    ...
    default:
      throw new Error('Valor de barras invalido');
  }
}
...
// Calcula a complexidade da senha para determinar a força
// Retorna um array contendo
// a força da s
// a descrição da força da senha a ser exibida e o número
// de barras da medida a serem preenchidas
const calcStrength = (passwordLength, charPoolSize) => {
  const strength = passwordLength * Math.log2(charPoolSize);

  if (strength < 25) {
    return ['Muito Fraca!', 1];
...
const validateInput = () => {
  // Pelo menos uma caixa está marcada
...
const copyPassword = async () => {
  ...
  passwordCopiedNotification.textContent = 'Copiado';

  // Fade out do texto depois de 1 segundo
  setTimeout(() => {
    passwordCopiedNotification.style.transition = 'all 1s';
    passwordCopiedNotification.style.opacity = 0;

    // Remove styles e texto antes do fade out
    setTimeout(() => {
...
// Inicializa o número de caracteres no carregamento da página especificado
// pelo range input
charCount.textContent = lengthSlider.value;
